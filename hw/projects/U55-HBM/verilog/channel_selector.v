...
module channel_selector (
    ...
    reg [1:0]		                rr_sid, r_sid, ns_sid; // indicates target PC
...
//    			ns_ch_dfi_dw_rddata_p1  		= dfi_8_dw_rddata_p1;
    	...
    	endcase
        if (hbm_enabled_channels[0] == 1'b1) begin
            ns_0_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[1] == 1'b1) begin
            ns_1_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[2] == 1'b1) begin
            ns_2_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[3] == 1'b1) begin
            ns_3_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[4] == 1'b1) begin
            ns_4_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[5] == 1'b1) begin
            ns_5_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[6] == 1'b1) begin
            ns_6_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[7] == 1'b1) begin
            ns_7_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[8] == 1'b1) begin
            ns_8_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[9] == 1'b1) begin
            ns_9_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[10] == 1'b1) begin
            ns_10_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[11] == 1'b1) begin
            ns_11_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[12] == 1'b1) begin
            ns_12_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[13] == 1'b1) begin
            ns_13_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[14] == 1'b1) begin
            ns_14_cmd_type = cmd_type;
        end
        if (hbm_enabled_channels[15] == 1'b1) begin
            ns_15_cmd_type = cmd_type;
        end
    end
    ...
    always @ (posedge dfi_clk) begin
        if (!dfi_rst_n) begin
            ...
            r_BA4 <= 0;
            r_sid <= 0;
            rr_0_cmd_type  <= `RNOP & `RNOP;
            rr_0_cmd_type  <= `RNOP & `RNOP;
            rr_1_cmd_type  <= `RNOP & `RNOP;
            rr_2_cmd_type  <= `RNOP & `RNOP;
            rr_3_cmd_type  <= `RNOP & `RNOP;
            rr_4_cmd_type  <= `RNOP & `RNOP;
            rr_5_cmd_type  <= `RNOP & `RNOP;
            rr_6_cmd_type  <= `RNOP & `RNOP;
            rr_7_cmd_type  <= `RNOP & `RNOP;
            rr_8_cmd_type  <= `RNOP & `RNOP;
            rr_9_cmd_type  <= `RNOP & `RNOP;
            rr_10_cmd_type <= `RNOP & `RNOP;
            rr_11_cmd_type <= `RNOP & `RNOP;
            rr_12_cmd_type <= `RNOP & `RNOP;
            rr_13_cmd_type <= `RNOP & `RNOP;
            rr_14_cmd_type <= `RNOP & `RNOP;
            rr_15_cmd_type <= `RNOP & `RNOP;
            rr_row_addr <= 0;
            rr_col_addr <= 0;
            rr_ba_addr <= 0;
            rr_i_wrdata <= 0;
    ...
    assign o_ch_ready = rr_ch_dfi_out_rst_n;
