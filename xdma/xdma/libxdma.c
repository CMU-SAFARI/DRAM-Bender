...
static void transfer_destroy(struct xdma_dev *xdev, struct xdma_transfer *xfer)
{
	...
	if (xfer->last_in_request && (xfer->flags & XFER_FLAG_NEED_UNMAP)) {
		...
		if (sgt->nents) {
			xdma_dma_unmap_sg(xdev->pdev, sgt->sgl, sgt->nents,
					   xfer->dir);
			sgt->nents = 0;
		}
	}
			...
			bool dma_mapped, int timeout_ms)
{
	...
	if (!dma_mapped) {
		sgt->nents = xdma_dma_map_sg(xdev->pdev, sgt->sgl,
					     sgt->orig_nents, dir);
		if (!sgt->nents) {
...
unmap_sgl:
	if (!dma_mapped && sgt->nents) {
		xdma_dma_unmap_sg(xdev->pdev, sgt->sgl, sgt->orig_nents, dir);
		sgt->nents = 0;
	}
			 ...
			 struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
	...
	if (!dma_mapped) {
		nents = xdma_dma_map_sg(xdev->pdev, sg, sgt->orig_nents, dir);
		if (!nents) {
			pr_info("map sgl failed, sgt 0x%p.\n", sgt);
			return -EIO;
		}
				...
				bool dma_mapped, int timeout_ms)
{
	...
	while (nents) {
		...
		if (rv < 0) {
			...
			if (!dma_mapped && sgt->nents) {
				xdma_dma_unmap_sg(xdev->pdev, sgt->sgl,
						   sgt->orig_nents, dir);
				sgt->nents = 0;
			}
...
static int set_dma_mask(struct pci_dev *pdev)
{
	...
	if (!xdma_set_dma_mask(pdev, DMA_BIT_MASK(64))) {
		/* query for DMA transfer */
		/* @see Documentation/DMA-mapping.txt */
		dbg_init("xdma_set_dma_mask()\n");
		/* use 64-bit DMA */
		dbg_init("Using a 64-bit DMA mask.\n");
		xdma_set_coherent_dma_mask(pdev, DMA_BIT_MASK(64));
	} else if (!xdma_set_dma_mask(pdev, DMA_BIT_MASK(32))) {
		dbg_init("Could not set 64-bit DMA mask.\n");
		xdma_set_coherent_dma_mask(pdev, DMA_BIT_MASK(32));
		/* use 32-bit DMA */
