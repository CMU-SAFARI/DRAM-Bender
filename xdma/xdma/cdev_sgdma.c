...
static void async_io_handler(unsigned long  cb_hndl, int err)
{
	...
	if (caio->cmpl_cnt == caio->req_cnt) {
		res = caio->res;
		res2 = caio->res2;
		xdma_iocb_complete(caio->iocb, res, res2);
skip_tran:
...
skip_dev_lock:
	xdma_iocb_complete(caio->iocb, numbytes, -EBUSY);
	kmem_cache_free(cdev_cache, caio);
}
...
static int char_sgdma_map_user_buf_to_sgl(struct xdma_io_cb *cb, bool write)
{
	...
	rv = xdma_get_user_pages_fast((unsigned long)buf, pages_nr, write,
				      cb->pages);
...
{
	return cdev_aio_write(iocb, xdma_iov_iter_iov(io),
			xdma_iov_iter_nr_segs(io), xdma_iov_iter_offset(io));
}

static ssize_t cdev_read_iter(struct kiocb *iocb, struct iov_iter *io)
{
	return cdev_aio_read(iocb, xdma_iov_iter_iov(io),
			xdma_iov_iter_nr_segs(io), xdma_iov_iter_offset(io));
}
#endif
