...
def _load_templates() -> tuple[list[str], list[str], dict]:
    ...
    for module_info in sorted(pkgutil.iter_modules(__path__), key=lambda item: item.name):
        ...
        if module_info.ispkg:
            raise ImportError(
                f"draminspector.builtin_programs does not support public subpackages; found {module_name!r}."
            )
        if module_name in seen_names:
            raise ImportError(
                f"draminspector.builtin_programs discovered duplicate public template name {module_name!r}."
            )
        ...
        if template is None:
            raise ImportError(
                f"draminspector.builtin_programs module {module_name!r} must define a top-level callable "
                f"named {module_name!r}."
            )
        if not callable(template) and not isinstance(template, _MetaBoundTemplate):
            raise TypeError(
                f"draminspector.builtin_programs symbol {module_name!r} must be callable, got "
