...
_TRACE_MODE: ContextVar[bool | None] = ContextVar(
    "draminspector_program_template_trace_mode",
    default=None,
)
...
def get_jit_cache_dir() -> Path:
    ...
    env_value = os.environ.get("DRAMINSPECTOR_JIT_CACHE_DIR")
    if env_value:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if qualname not in _FALLBACK_WARNED:
                ...
                warnings.warn(
                    f"draminspector JIT fell back to interpreted mode for "
...
def _specialization_metadata(*, function, trace_ops, scalar_names) -> dict[str, Any]:
    ...
    key_material = {
        ...
        "draminspector_core": _core_freshness_marker(),
    }
    key = hashlib.sha256(
...
def _render_plugin_source(trace_ops, scalar_names) -> str:
    ...
    lines = [
        ...
        "",
        "extern \"C\" uint32_t draminspector_template_plugin_abi_version() {",
